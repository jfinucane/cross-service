...
Notes
  ...
  in bash shell => export HOST=www.jimrails.com 

Words: 

  Default dictionary is sowpods; to override:         &dictionary=test
  Default start word is 0; to override:               &offset=500 
  Default 300 words returned at a time;  to override:   &page_size=1000

  Services                                            
    curl anagrams.$HOST/sample.json        - returns -           list
    curl startswith.$HOST/sample.json      - returns -           list
    curl contains.$HOST/demo.json          - returns -           list
    curl suggestions.$HOST/demo.json       - returns -           lists in hash, by length @wip
    curl autocomplete.$HOST/demo.json      - returns -           list @wip

  

Get anagrams from default dictionary

    curl $HOST/anagrams/demo.json

    => ["mode","dome","demo"]

  or specify dictionary 

    curl $HOST/anagrams/demo.json?dictionary=sowpods
